...
public class SnakeHead
{
    ...
    public SnakeHead(float levelSizeX, float levelSizeY, int length, Color headColor, Color bodyColor)
    {
        ...
        Body.Add(new SnakeBody(Position.X + length - 1, Position.Y, Direction.Left, bodyColor));

        while (Body.Count < length - 1)
        {
            SnakeBody previous = Body.Last();
            Body.Add(new SnakeBody(previous.Position.X - 1, Position.Y, Direction.Left, bodyColor, previous));
        }
